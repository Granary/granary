...
typedef struct { reg_t xax , xbx , xcx , xdx , xsi , xdi , xbp , xsp , xflags , pc ; reg_t r8 , r9 , r10 , r11 , r12 , r13 , r14 , r15 ; dr_ymm_t ymm ; } dr_mcontext_t ;

typedef dr_mcontext_t priv_mcontext_t ;

typedef struct { dr_mcontext_t mcontext ; } unprotected_context_t ;
...
enum { LINK_DIRECT = 0x0001 , LINK_INDIRECT = 0x0002 , LINK_RETURN = 0x0004 , LINK_CALL = 0x0008 , LINK_JMP = 0x0010 , LINK_FAR = 0x0020 , LINK_SELFMOD_EXIT = 0x0040 , LINK_TRACE_CMP = 0x0100 , LINK_NI_SYSCALL_INT = 0x0200 , LINK_NI_SYSCALL = 0x0400 , LINK_FINAL_INSTR_SHARED_FLAG = LINK_NI_SYSCALL , LINK_FRAG_OFFS_AT_END = 0x0800 , LINK_END_OF_LIST = 0x1000 , LINK_FAKE = 0x2000 , LINK_LINKED = 0x4000 , LINK_SEPARATE_STUB = 0x8000 } ;
...
struct _opnd_t { byte kind ; opnd_size_t size ; union { ushort far_pc_seg_selector ; reg_id_t segment : 8 ; ushort disp ; } seg ; union { ptr_int_t immed_int ; float immed_float ; app_pc pc ; instr_t * instr ; reg_id_t reg ; struct { int disp ; reg_id_t base_reg : 8 ; reg_id_t index_reg : 8 ; byte scale : 4 ; byte encode_zero_disp : 1 ; byte force_full_disp : 1 ; byte disp_short_addr : 1 ; } base_disp ; void * addr ; } value ; } ;
...
enum { INSTR_DIRECT_EXIT = LINK_DIRECT , INSTR_INDIRECT_EXIT = LINK_INDIRECT , INSTR_RETURN_EXIT = LINK_RETURN , INSTR_CALL_EXIT = LINK_CALL , INSTR_JMP_EXIT = LINK_JMP , INSTR_IND_JMP_PLT_EXIT = ( INSTR_JMP_EXIT | INSTR_CALL_EXIT ) , INSTR_FAR_EXIT = LINK_FAR , INSTR_BRANCH_SELFMOD_EXIT = LINK_SELFMOD_EXIT , INSTR_TRACE_CMP_EXIT = LINK_TRACE_CMP , INSTR_NI_SYSCALL_INT = LINK_NI_SYSCALL_INT , INSTR_NI_SYSCALL = LINK_NI_SYSCALL , INSTR_NI_SYSCALL_ALL = ( LINK_NI_SYSCALL | LINK_NI_SYSCALL_INT ) , EXIT_CTI_TYPES = ( INSTR_DIRECT_EXIT | INSTR_INDIRECT_EXIT | INSTR_RETURN_EXIT | INSTR_CALL_EXIT | INSTR_JMP_EXIT | INSTR_FAR_EXIT | INSTR_BRANCH_SELFMOD_EXIT | INSTR_TRACE_CMP_EXIT | INSTR_NI_SYSCALL_INT | INSTR_NI_SYSCALL ) , INSTR_OPERANDS_VALID = 0x00010000 , INSTR_FIRST_NON_LINK_SHARED_FLAG = INSTR_OPERANDS_VALID , INSTR_EFLAGS_VALID = 0x00020000 , INSTR_EFLAGS_6_VALID = 0x00040000 , INSTR_RAW_BITS_VALID = 0x00080000 , INSTR_RAW_BITS_ALLOCATED = 0x00100000 , INSTR_DO_NOT_MANGLE = 0x00200000 , INSTR_HAS_CUSTOM_STUB = 0x00400000 , INSTR_IND_CALL_DIRECT = 0x00800000 , INSTR_CLOBBER_RETADDR = 0x02000000 , INSTR_HOT_PATCHABLE = 0x04000000 , INSTR_DO_NOT_EMIT = 0x10000000 , INSTR_RIP_REL_VALID = 0x20000000 , INSTR_X86_MODE = 0x40000000 , INSTR_OUR_MANGLING = 0x80000000 } ;
...
enum { INVALID = OP_LAST + 1 , PREFIX , ESCAPE , FLOAT_EXT , EXTENSION , PREFIX_EXT , REP_EXT , REPNE_EXT , MOD_EXT , RM_EXT , SUFFIX_EXT , X64_EXT , ESCAPE_3BYTE_38 , ESCAPE_3BYTE_3a , REX_EXT , VEX_PREFIX_EXT , VEX_EXT , VEX_L_EXT , VEX_W_EXT } ;

typedef struct decode_info_t { uint prefixes ; byte seg_override ; byte modrm ; byte mod ; byte reg ; byte rm ; bool has_sib ; byte scale ; byte index ; byte base ; bool has_disp ; int disp ; opnd_size_t size_immed ; opnd_size_t size_immed2 ; ptr_int_t immed ; ptr_int_t immed2 ; byte * start_pc ; byte * final_pc ; uint len ; byte * disp_abs ; bool x86_mode ; byte * orig_pc ; bool data_prefix ; bool rep_prefix ; bool repne_prefix ; byte vex_vvvv ; bool vex_encoded ; ptr_int_t cur_note ; bool has_instr_opnds ; } decode_info_t ;
...
typedef struct _clean_call_info_t { void * callee ; uint num_args ; bool save_fpstate ; bool opt_inline ; bool should_align ; bool save_all_regs ; bool skip_save_aflags ; bool skip_clear_eflags ; uint num_xmms_skip ; bool xmm_skip [ 0 ] ; uint num_regs_skip ; bool reg_skip [ ( 1 + ( DR_REG_R15 - DR_REG_RAX ) ) ] ; bool preserve_mcontext ; void * callee_info ; instrlist_t * ilist ; } clean_call_info_t ;

enum { FCACHE_ENTER_TARGET_SLOT = 0 , MANGLE_NEXT_TAG_SLOT = 0 , DIRECT_STUB_SPILL_SLOT = 0 , MANGLE_RIPREL_SPILL_SLOT = 0 , INDIRECT_STUB_SPILL_SLOT = 0 , MANGLE_FAR_SPILL_SLOT = 0 , MANGLE_XCX_SPILL_SLOT = 0 , DCONTEXT_BASE_SPILL_SLOT = 0 , PREFIX_XAX_SPILL_SLOT = 0 } ;
