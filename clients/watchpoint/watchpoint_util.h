...
typedef unsigned int uint;
    ...
    public:
        ...
        inline void replace_operand(granary::instruction &in){
            unsigned int i = 0, max = in.num_destinations();
            dynamorio::opnd_t opnd;
            dynamorio::instr_t *instr = (dynamorio::instr_t*)in;
            for(i = 0; i < max; i++){
                opnd = dynamorio::instr_get_dst(instr, i);
                if(opnd.kind == dynamorio::BASE_DISP_kind){
                    dynamorio::instr_set_dst(instr, i, replacement_operand);
                }
            }
            ...
            max = in.num_sources();
            for(i = 0; i < max; i++){
                opnd = dynamorio::instr_get_src(instr, i);
                if(opnd.kind == dynamorio::BASE_DISP_kind){
                    dynamorio::instr_set_src(instr, i, replacement_operand);
                }
            }
